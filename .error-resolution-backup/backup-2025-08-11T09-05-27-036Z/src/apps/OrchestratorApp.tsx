...
import { orchestratorConfig as defaultConfig } from '../services/orchestratorService';
import { controlPanelService } from '../services/controlPanelService';
import type { LLMProvider, ControlPanelState } from '../types';

const Section: FC<{ title: any; children: React.ReactNode; className?: any }> = ({ title, children, className = '' }) => (
...
const ProviderCard: FC<{ provider: LLMProvider, isEnabled: boolean }> = ({ provider, isEnabled }) => (
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="bg-slate-800 p-3 rounded-md">
                            ...
                            <ul className="space-y-1 text-xs text-slate-400 list-disc list-inside">
                                ...
                                <li>Confidence Threshold: {defaultConfig.fallbackStrategy.confidenceThreshold}</li>
                                <li>Fallback on Error: {defaultConfig.fallbackStrategy.fallbackOnError ? 'Yes' : 'No'}</li>
                                <li>Fallback on Timeout: {defaultConfig.fallbackStrategy.fallbackOnTimeout ? 'Yes' : 'No'}</li>
                            </ul>
                        </div>
                         <div className="bg-slate-800 p-3 rounded-md">
                            <h4 className="font-bold text-white mb-2">Cost Optimization</h4>
                             <ul className="space-y-1 text-xs text-slate-400 list-disc list-inside">
                                <li>Monthly Budget: ${liveState.orchestrator.monthlyBudget}</li>
                                <li className={defaultConfig.costOptimization.preferLocalModels ? 'text-green-400' : ''}>
                                    Prefer Local Models: {defaultConfig.costOptimization.preferLocalModels ? 'Yes' : 'No'}
                                </li>
                                <li className={defaultConfig.costOptimization.cacheResponses ? 'text-green-400' : ''}>
                                    Cache Responses: {defaultConfig.costOptimization.cacheResponses ? `Yes (TTL: ${defaultConfig.costOptimization.cacheTTL}s)` : 'No'}
                                </li>
                             </ul>
                        </div>
                    </div>
