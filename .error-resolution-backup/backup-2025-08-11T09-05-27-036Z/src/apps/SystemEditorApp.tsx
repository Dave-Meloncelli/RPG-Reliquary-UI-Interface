...
const SystemEditorApp: React.FC = () => {
    ...
    const handleOpenFile = useCallback((path: any, name: any) => {
    ...
    const handleCloseFile = useCallback((path: any) => {
    ...
    const handleContentChange = useCallback((path: any, newContent: any) => {
      setOpenFiles(prev => prev.map(f => (f.path === path ? { ...f, content: newContent } : f)));
    }, []);

    const handleSaveActiveFile = useCallback((path: any) => {
