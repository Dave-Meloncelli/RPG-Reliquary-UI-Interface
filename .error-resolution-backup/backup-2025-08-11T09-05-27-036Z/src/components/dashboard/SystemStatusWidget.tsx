...
import { streamErduEvents } from '../../services/erduService';
...
const StatusMeter: FC<{ label: string; value: number; colorClass: string }> = ({ label, value, colorClass }) => (
    <div>
        <div className="flex justify-between items-center mb-1 text-sm">
            ...
            <span className={`font-mono font-bold ${colorClass.replace('bg-', 'text-')}`}>{value.toFixed(0)}%</span>
        </div>
...
const SystemStatusWidget: FC = () => {
    ...
    const threatColor = status.threatLevel > 70 ? 'bg-red-500' : status.threatLevel > 40 ? 'bg-yellow-500' : 'bg-green-500';

    return (
        <div className="h-full flex flex-col justify-center gap-6">
            <StatusMeter label="System Integrity" value={status.integrity} colorClass={integrityColor} />
            <StatusMeter label="Threat Level" value={status.threatLevel} colorClass={threatColor} />
        </div>
    );
};
