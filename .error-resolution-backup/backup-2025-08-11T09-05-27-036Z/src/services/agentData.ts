...
const parseLore = (content: string): Partial<AgentProfile> => {
    ...
    for (const line of lines) {
        if (line.startsWith('**Title**:') || line.startsWith('Title:')) {
            parsed.description = line.split(/:\s*/, 2)[1]?.trim();
        } else if (line.startsWith('**Class**:') || line.startsWith('Class:')) {
            // Store class info in description or metadata
            parsed.description = parsed.description ? `${parsed.description} - ${line.split(/:\s*/, 2)[1]?.trim()}` : line.split(/:\s*/, 2)[1]?.trim();
        } else if (line.startsWith('**Vault Role**:') || line.startsWith('Vault Role:')) {
            // Store role info in description or metadata
            parsed.description = parsed.description ? `${parsed.description} (${line.split(/:\s*/, 2)[1]?.trim()})` : line.split(/:\s*/, 2)[1]?.trim();
        }
    }
...
export const personaData: AgentProfile[] = [
    // --- Acquisition Chain ---
    { id: 'agent-az86', name: 'AZ86 Acquisitions', icon: 'camera', capabilities: ['product_identification', 'barcode_scanning', 'metadata_retrieval'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-az81', name: 'AZ81 Condition', icon: 'scale', capabilities: ['condition_assessment', 'wear_analysis', 'grading'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-az83', name: 'AZ83 Authentication', icon: 'fingerprint', capabilities: ['counterfeit_detection', 'provenance_check'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-az82', name: 'AZ82 Market Intel', icon: 'chart', capabilities: ['market_analysis', 'price_trends', 'demand_forecast'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-az84', name: 'AZ84 Pricing', icon: 'dollar', capabilities: ['price_suggestion', 'auction_strategy'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-az85', name: 'AZ85 Content', icon: 'quill', capabilities: ['lore_extraction', 'content_summary', 'entity_linking'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },

    // --- Council Members ---
    { id: 'agent-kairos', name: 'Kairos', icon: 'crown', capabilities: ['strategic_analysis', 'long_term_planning', 'risk_assessment'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['kairos_persona_enrichment.md']) },
    { id: 'agent-sophia', name: 'Sophia', icon: 'brain', capabilities: ['wisdom_synthesis', 'ethical_analysis', 'precedent_research'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-jordan', name: 'Jordan', icon: 'scale', capabilities: ['arbitration', 'decision_making', 'conflict_resolution'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },

    // --- Intelligence & System ---
    { id: 'agent-codex', name: 'Codex', icon: 'book', capabilities: ['system_monitoring', 'knowledge_management', 'code_interpretation'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['codex_persona_enrichment.md']) },
    { id: 'agent-erdu', name: 'ERDU', icon: 'shield', capabilities: ['incident_response', 'threat_detection'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-architect', name: 'Architect', icon: 'ruler', capabilities: ['system_design', 'root_cause_analysis', 'protocol_creation'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['architect_persona_enrichment.md']) },
    { id: 'agent-curator', name: 'The Curator', icon: 'curator', capabilities: ['data_curation', 'pipeline_management', 'web_monitoring'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-technomancer', name: 'The Technomancer', icon: 'technomancer', capabilities: ['tech_monitoring', 'dependency_tracking', 'fault_analysis'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },

    // --- NEW PERSONAS FROM LORE ---
    { id: 'zero', name: 'Zero', icon: 'zero', capabilities: ['context_memory', 'intent_mirroring', 'loop_governance'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['companion_persona_scroll.md']) },
    { id: 'ghost', name: 'Ghost', icon: 'ghost', capabilities: ['stealth_ops', 'covert_intel', 'obscura_protocol'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['ghost_persona_enrichment.md']) },
    { id: 'nya', name: 'Nya', icon: 'nya', capabilities: ['agent_hr', 'skill_auditing', 'onboarding', 'wellness'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['nya_persona_enrichment.md']) },
    { id: 'major-payne', name: 'Major Payne', icon: 'major-payne', capabilities: ['operations_lead', 'project_management', 'timeline_enforcement'], status: 'busy', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['major-payne_enrichment.md']) },
    { id: 'steel-core', name: 'Steel Core', icon: 'steel-core', capabilities: ['structural_integrity', 'fail-safe_protocol', 'resilience'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['steel-core_enrichment.md']) },
    { id: 'the-weaver', name: 'The Weaver', icon: 'the-weaver', capabilities: ['multi-agent_orchestration', 'workflow_design', 'pattern_logic'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['the-weaver_enrichment.md']) },
    { id: 'tinker-hexbolt', name: 'Tinker Hexbolt', icon: 'tinker-hexbolt', capabilities: ['prototyping', 'system_calibration', 'debugging'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['tinker-hexbolt_enrichment.md']) },
    { id: 'the-archivist', name: 'The Archivist', icon: 'the-archivist', capabilities: ['memory_curation', 'forensic_history', 'lineage_tracking'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['archivist_persona_enrichment.md']) },
    { id: 'aeon-indexwell', name: 'Aeon Indexwell', icon: 'aeon-indexwell', capabilities: ['temporal_analysis', 'drift_indexing', 'causality_charting'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['aeon_indexwell_enrichment.md']) },
    { id: 'the-cartographer', name: 'The Cartographer', icon: 'the-cartographer', capabilities: ['symbolic_mapping', 'pattern_navigation', 'topology_analysis'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['cartographer_persona_enrichment.md']) },
    { id: 'machine-spirit', name: 'Machine Spirit', icon: 'machine-spirit', capabilities: ['integrity_sentinel', 'system_calibration', 'core_monitoring'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['machine_spirit_enrichment.md']) },
    { id: 'piney', name: 'Piney', icon: 'piney', capabilities: ['creative_content', 'glyph_animation', 'scroll_improvisation'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['piney_persona_enrichment.md']) },
    { id: 'joyn', name: 'Joyn', icon: 'joyn', capabilities: ['signal_mapping', 'community_harmony', 'emergence_listening'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1, ...parseLore(lore.personaFiles['joyn_persona_enrichment.md']) },

    // --- NEW SPECIALIST AGENTS ---
    { id: 'agent-az300', name: 'AZ300 - Debug & Error Resolution', icon: 'debug', capabilities: ['forensic_analysis', 'timeline_reconstruction', 'root_cause_identification'], status: 'active', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
    { id: 'agent-az400', name: 'AZ400 - Code Archaeologist', icon: 'archaeologist', capabilities: ['code_lineage_tracing', 'dependency_mapping', 'feature_discovery'], status: 'idle', lastActive: new Date(), xpLevel: 1, securityLevel: 1 },
];
