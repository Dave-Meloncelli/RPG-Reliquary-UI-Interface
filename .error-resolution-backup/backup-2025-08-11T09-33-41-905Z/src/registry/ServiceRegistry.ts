...
export class ServiceRegistry {
  ...
  validate(): ValidationResult {
    ...
    for (const service of this.getAll()) {
      const endpoints = service.api.map(a => a.endpoint);
      const endpointDuplicates = endpoints.filter((endpoint, index) => endpoints.indexOf(endpoint) !== index);
      if (endpointDuplicates.length > 0) {
        errors.push(`Service ${service.name}: Duplicate API endpoints found: ${endpointDuplicates.join(', ')}`);
      }
    }
  ...
  private isExternalDependency(dep: string): boolean {
    ...
    const externalPrefixes = ['@', 'react', 'lodash', 'axios', 'express', 'fastapi'];
