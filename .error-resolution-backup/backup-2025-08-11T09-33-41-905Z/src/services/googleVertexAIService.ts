...
export class GoogleVertexAIService {
  ...
  async analyzeABTest(testId: string): Promise<{
    ...
    abTest.variants.forEach(variant => {
      variantResultsMap.set(variant.id, results.filter(r => r.variantId === variant.id));
    });
    ...
    for (const [variantId, variantResults] of variantResultsMap) {
      ...
      const avgResponseTime = variantResults.reduce((sum: number, r: ABTestResult) => sum + r.metrics.responseTime, 0) / variantResults.length;
      const avgUserSatisfaction = variantResults.reduce((sum: number, r: ABTestResult) => sum + (r.metrics.userSatisfaction || 0), 0) / variantResults.length;
      const avgConversionRate = variantResults.reduce((sum: number, r: ABTestResult) => sum + (r.metrics.conversionRate || 0), 0) / variantResults.length;
      const errorRate = variantResults.reduce((sum: number, r: ABTestResult) => sum + r.metrics.errorRate, 0) / variantResults.length;
