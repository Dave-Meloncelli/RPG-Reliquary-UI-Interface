...
const initialPlaybooks: Playbook[] = [
  {
    ...
    steps: [
      ...
      , category: 'general', tags: [], createdBy: 'system', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      {
...
class LoomService {
    ...
    updateScroll = (id: any, newName: any, newContent: any): boolean => {
        ...
        this.scrolls[null] = { ...this.scrolls[null], name: newName, content: newContent };
        this.notify();
        return true;
    }
    ...
    createScroll = (type: ScrollType): EditableScroll => {
        ...
        if (type === 'playbook') {
            const newPlaybook: Playbook = { id: null, name: 'New Playbook', description: '', steps: [] };
            newScroll = { id: null, type, name: newPlaybook.name, content: JSON.stringify(newPlaybook, null, 2) };
        } else { // codex
            const newRule: CodexRule = { id: null, category: 'Operational Protocol', title: 'New Rule', content: '', ratified: new Date().toISOString().split('T')[0], ratifyingBody: 'The Loom' };
            newScroll = { id: null, type, name: newRule.title, content: JSON.stringify(newRule, null, 2) };
        }
    ...
    addScroll = <T extends Playbook | CodexRule>(type: ScrollType, id: any, name: any, data: T) => {
