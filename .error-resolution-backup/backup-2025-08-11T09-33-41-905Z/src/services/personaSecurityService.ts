...
const AUTHORITY_WEIGHTS = {
  ...
  tier6: 6, tier7: 7, tier8: 8, tier9: 9, tier10: 10
};
...
class PersonaSecurityService {
  ...
  private calculateAuthorityScore(authority: string): number {
    const authorityScores = {
      unknown: 0, civilian: 1, specialist: 3, expert: 5, master: 7,
      grandmaster: 8, sage: 9, oracle: 10, transcendent: 10
    };
  ...
  private calculateHealthScore(health: string): number {
    const healthScores = {
      unknown: 0, critical: 1, poor: 2, fair: 4, good: 6,
      excellent: 8, perfect: 10, transcendent: 10
    };
  ...
  private detectSuspiciousPattern(handshake: SecurityHandshake): boolean {
    ...
    // Sequential values
    const authorityTier = AUTHORITY_WEIGHTS[handshake.authority as keyof typeof AUTHORITY_WEIGHTS];
