...
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'http: any;
...
class SymposiumService {
  ...
  private sessions: any, SymposiumSession> = new Map();
  private agents: any
    {
      id: any,
      name: any,
  ...
  async startSymposium(topic: any, openingMessage: any
  ...
  async getSymposiumStats(): Promise<{
    ...
    const participants = new Set(this.messages?.map((m: any;
