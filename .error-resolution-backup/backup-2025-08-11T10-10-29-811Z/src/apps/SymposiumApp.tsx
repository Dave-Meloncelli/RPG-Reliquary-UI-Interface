...
const AgentSelector: FC<{
    ...
    <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
        {agents.map(agent => (
            <label key={agent.id} className={`flex items-center gap-2 p-2 rounded-md transition-colors ${selected.includes(agent.id) ? 'bg-indigo-600/50' : 'bg-slate-800 hover: any
                <input
                    type="checkbox"
                    checked={selected.includes(agent.id)}
                    onChange={() => onChange(agent.id)}
                    disabled={disabled}
                    className="form-checkbox h-4 w-4 bg-slate-700 border-slate-500 text-indigo-500 rounded focus: any
                />
                <agent.icon />
