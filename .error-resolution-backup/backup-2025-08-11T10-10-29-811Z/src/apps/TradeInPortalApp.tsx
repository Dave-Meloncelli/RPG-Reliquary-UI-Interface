...
const TradeInPortalApp: React.FC = () => {
  ...
  const [currentStep, setCurrentStep] = useState<'welcome' | 'upload' | 'analysis' | 'complete'>('welcome');
    ...
    files.forEach((file, index) => {
      ...
      reader.onload = (e) => {
        ...
        if (index === files.length - 1) {
          setTradeInBooks(prev => [...prev, ...newBooks.filter(book => book.backImage)]);
        }
      };
  ...
  const analyzeBooks = useCallback(async () => {
    ...
    for (let i = 0; i < tradeInBooks.length; i++) {
      ...
      setTradeInBooks(prev => prev.map(b => 
        b.id === book.id ? { ...b, status: 'analyzing' } : b
      ));
      ...
      try {
        ...
        const analysis = {
          ...
          condition: ['excellent', 'very_good', 'good', 'fair'][Math.floor(Math.random() * 4)],
          estimatedValue: 25 + Math.floor(Math.random() * 150),
          tradeValue: 0
        };

        analysis.tradeValue = Math.floor(analysis.estimatedValue * (0.3 + Math.random() * 0.2));
    ...
    const total = tradeInBooks.reduce((sum, book) => sum + (book.tradeValue || 0), 0);
  ...
  const getStatusIcon = (status: TradeInBook['status']) => {
    switch (status) {
      ...
      case 'analyzing': return <div className="w-4 h-4 bg-blue-500 rounded-full animate-pulse"></div>;
      case 'valued': return <div className="w-4 h-4 bg-yellow-500 rounded-full"></div>;
      case 'recommended': return <div className="w-4 h-4 bg-green-500 rounded-full"></div>;
    }
  };
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-blue-200">
            Turn your old RPG books into new adventures! Get instant valuations and trade recommendations.
          </p>
        </div>
        ...
        <div className="flex justify-center mb-8">
          <div className="flex items-center space-x-4">
            {['welcome', 'upload', 'analysis', 'complete'].map((step, index) => (
              <div key={step} className="flex items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                  currentStep === step ? 'bg-blue-600' : 
                  ['welcome', 'upload', 'analysis', 'complete'].indexOf(currentStep) > index ? 'bg-green-600' : 'bg-gray-600'
                }`}>
                  {index + 1}
                </div>
                {index < 3 && (
        ...
        {currentStep === 'welcome' && (
          <div className="bg-black/30 rounded-lg p-8 text-center">
            <h2 className="text-2xl font-bold mb-4">Welcome to the RPG Trade-In Portal!</h2>
            <p className="text-lg text-blue-200 mb-6">
              We're building the ultimate RPG community hub. Trade in your old books and help us create 
              the most comprehensive RPG database in the world.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-black/20 p-4 rounded-lg">
                <h3 className="text-lg font-semibold mb-2">📸 Easy Photo Upload</h3>
                <p className="text-sm text-gray-300">Just snap front/back cover photos and we'll do the rest</p>
              </div>
              <div className="bg-black/20 p-4 rounded-lg">
                <h3 className="text-lg font-semibold mb-2">🤖 AI-Powered Analysis</h3>
                <p className="text-sm text-gray-300">Instant valuations with market data and condition assessment</p>
              </div>
              <div className="bg-black/20 p-4 rounded-lg">
                <h3 className="text-lg font-semibold mb-2">💰 Fair Trade Values</h3>
                <p className="text-sm text-gray-300">Get competitive trade-in values based on real market data</p>
              </div>
            </div>
            <button 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div>
                <div className="bg-black/20 p-6 rounded-lg border-2 border-dashed border-blue-500">
                  ...
                  <p className="text-sm text-gray-300 text-center">
                    Upload front and back cover photos for each book.<br/>
                ...
                {tradeInBooks.length > 0 && (
                  <div className="mt-6">
                    <h3 className="text-lg font-semibold mb-3">Uploaded Books ({tradeInBooks.length})</h3>
                    <div className="space-y-2">
                      {tradeInBooks.map(book => (
                        <div key={book.id} className="flex items-center gap-3 bg-black/20 p-3 rounded-lg">
                          <div className="w-12 h-16 bg-gray-700 rounded flex items-center justify-center text-xs">
                            📚
                          </div>
                          <div className="flex-grow">
                            <p className="text-sm font-semibold">Book {book.id.slice(-6)}</p>
                            <p className="text-xs text-gray-400">Front & Back covers uploaded</p>
                          </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="bg-black/20 p-4 rounded-lg">
                    <h4 className="font-semibold text-blue-300 mb-2">Photo Quality</h4>
                  ...
                  <div className="bg-black/20 p-4 rounded-lg">
                    <h4 className="font-semibold text-green-300 mb-2">What We Accept</h4>
                    <ul className="text-sm text-gray-300 space-y-1">
        ...
        {currentStep === 'analysis' && (
          <div className="bg-black/30 rounded-lg p-8">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-blue-200">Our AI is analyzing your books...</p>
              <p className="text-sm text-gray-400">This may take a few minutes for each book</p>
            </div>
            
            <div className="space-y-4">
              {tradeInBooks.map(book => (
                <div key={book.id} className="flex items-center gap-4 bg-black/20 p-4 rounded-lg">
                  {getStatusIcon(book.status)}
                  <div className="flex-grow">
                    <p className="font-semibold">
                      {book.status === 'analyzing' ? 'Analyzing book...' : 
                       book.status === 'recommended' ? book.title || 'Book analyzed' : 
                       `Book ${book.id.slice(-6)}`}
                    </p>
                    {book.status === 'analyzing' && (
                      <p className="text-sm text-gray-400">Extracting title, publisher, condition, and market data...</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        {currentStep === 'complete' && (
          <div className="bg-black/30 rounded-lg p-8">
            <h2 className="text-2xl font-bold mb-6">Trade-In Recommendations</h2>
            
            {/* Summary */}
            <div className="bg-black/20 p-6 rounded-lg mb-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-2xl font-bold text-green-400">${totalTradeValue}</p>
                  <p className="text-sm text-gray-400">Total Trade Value</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-blue-400">{tradeInBooks.length}</p>
                ...
                <div key={book.id} className="bg-black/20 p-6 rounded-lg">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400 mb-2">{book.publisher}</p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-lg font-bold text-green-400">${book.estimatedValue}</span>
                      </div>
                    </div>
