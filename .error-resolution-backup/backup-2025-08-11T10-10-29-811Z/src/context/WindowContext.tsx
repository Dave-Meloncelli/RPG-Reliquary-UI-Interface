...
export const WindowProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const focusWindow = useCallback((id: string) => {
    setWindows(prev => {
      ...
      return prev.map(win => win.id === id ? { ...win, zIndex: newZIndex, isMinimized: false } : win);
    });
  }, []);
    ...
    setWindows(prev => {
      ...
      const updated = prev.map(w => {
        ...
        return {
          ...
          preMaximizeState: isMaximizing ? { x: w.x, y: w.y, width: w.width, height: w.height } : w.preMaximizeState,
  ...
  const value = { windows, openWindow, closeWindow, minimizeWindow, focusWindow, maximizeWindow, updateWindowPosition };
