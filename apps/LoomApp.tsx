...
const ScrollTypeSelector: FC<{ onSelect: (type: ScrollType) => void, selectedType: ScrollType }> = ({ onSelect, selectedType }) => (
...
const ScrollList: FC<{ scrolls: EditableScroll[], onSelect: (id: string) => void, selectedId: string | null, onCreate: () => void }> = ({ scrolls, onSelect, selectedId, onCreate }) => (
...
const LoomApp: React.FC = () => {
    ...
    const [scrolls, setScrolls] = useState<EditableScroll[]>(loomService.getScrolls(selectedType));
    const [selectedScroll, setSelectedScroll] = useState<EditableScroll | null>(null);
    ...
    useEffect(() => {
        const handleUpdate = () => {
            ...
            if (selectedScroll) {
                const stillExists = updatedScrolls.find(s => s.id === selectedScroll.id);
                if (!stillExists) {
    ...
    const handleSave = () => {
        ...
        const success = loomService.updateScroll(selectedScroll.id, editorName, editorContent);
        setSaveStatus(success ? 'success' : 'error');
        setTimeout(() => setSaveStatus('idle'), 2000);
    };

    const handleDelete = () => {
        if (selectedScroll && window.confirm(`Are you sure you want to delete "${selectedScroll.name}"?`)) {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
            <div className="p-4 border-b border-slate-700/50">
                ...
                <p className="text-sm text-slate-400">Integrated editor for system scrolls (Playbooks, Codex Rules).</p>
            </div>
            <div className="flex-grow flex min-h-0">
                {/* Left Panel: File Tree */}
                <div className="w-1/6 min-w-[150px] p-2 bg-slate-900/50 border-r border-slate-700/50">
                    <ScrollTypeSelector onSelect={setSelectedType} selectedType={selectedType} />
                </div>
