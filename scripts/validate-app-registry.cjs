...
class AppRegistryValidator {
  constructor() {
    console.log('Initializing AppRegistryValidator...');
    this.rootDir = process.cwd();
    this.constantsPath = path.join(this.rootDir, 'src/constants/constants.tsx');
    this.appsDir = path.join(this.rootDir, 'src/apps');
    this.errors = [];
    this.warnings = [];
    console.log('AppRegistryValidator initialized');
  }

  validateAppRegistry() {
    process.stdout.write('📱 Validating app registry...\n');
    process.stdout.write('Constants path: ' + this.constantsPath + '\n');
    process.stdout.write('Apps dir: ' + this.appsDir + '\n');
    process.stdout.write('Constants exists: ' + fs.existsSync(this.constantsPath) + '\n');
    process.stdout.write('Apps dir exists: ' + fs.existsSync(this.appsDir) + '\n');
  ...
  getRegisteredApps() {
    try {
      ...
      if (!appMatches) {
        console.log('No app matches found in constants file');
        return [];
      }
      ...
      console.log(`Found ${apps.length} registered apps:`, apps.map(app => app.id));
      return apps;
    } catch (error) {
  ...
  validateAppRegistry(registeredApps, appFiles) {
    ...
    if (!registeredApps || !Array.isArray(registeredApps)) {
      this.errors.push({
        type: 'invalid-registered-apps',
        message: 'Failed to parse registered apps'
      });
      return;
    }
...
// Run validation
console.log('Starting app registry validation...');
const validator = new AppRegistryValidator();
console.log('About to call validateAppRegistry...');
validator.validateAppRegistry();
console.log('validateAppRegistry completed'); 