...
const AgentCard: FC<{ agent: AgentProfile; isMasterDormant: boolean }> = ({
  agent,
  isMasterDormant,
}) => {
  const activityState = isMasterDormant
    ? "Dormant"
    : agent.status.activityState;

  return (
    <div
      className={`bg-slate-800/60 border border-slate-700 rounded-lg p-4 flex flex-col gap-4 shadow-lg hover:bg-slate-800 transition-all duration-200 ${isMasterDormant ? "opacity-50" : ""}`}
    >
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className={statusColors.health[agent.status.health]}>
                {activityState}
              </span>
              <div
                className={`w-2.5 h-2.5 rounded-full ${statusColors.activityState[activityState]}`}
              ></div>
            </div>
          </div>
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={statusColors.alignment[agent.status.alignment]}>
              {agent.status.alignment}
            </span>
          </div>
        </div>
      </div>
...
const AgentNetworkApp: React.FC = () => {
  ...
  const [masterStatus, setMasterStatus] = useState(
    controlPanelService.getState().agentMasterStatus,
  );
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          Live status of all operational agents.
        </p>
      </div>
