...
import { runBackupProcess } from '../services/backupService';
...
const StatusDisplay: FC<{ status: BackupStatus }> = ({ status }) => {
    ...
    const statusColor = isSuccess ? 'text-green-400' : isNeverRun ? 'text-slate-500' : 'text-red-400';
    ...
    return (
        <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 text-center">
            ...
            <p className={`text-xl font-bold mt-1 ${statusColor}`}>{status.lastBackupStatus}</p>
            {status.lastBackupTimestamp && <p className="text-xs text-slate-500 mt-1">at {status.lastBackupTimestamp}</p>}
        </div>
    );
};
...
const BackupApp: React.FC = () => {
    const [status, setStatus] = useState<BackupStatus>({ lastBackupStatus: 'Never run', lastBackupTimestamp: null });
    ...
    const handleRunBackup = useCallback(async () => {
        ...
        try {
            ...
            setLogs(prev => [...prev, {id: 999, timestamp: new Date().toLocaleTimeString(), message: "ERROR: The backup process failed unexpectedly."}]);
        } finally {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
             <div className="p-4 border-b border-slate-700/50">
                ...
                <p className="text-sm text-slate-400">Manage and monitor system backup protocols.</p>
            </div>
