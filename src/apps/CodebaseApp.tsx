...
const Section: FC<{ title: any; children: React.ReactNode }> = ({
  ...
  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 flex flex-col h-full overflow-hidden">
    ...
    <div className="flex-grow overflow-y-auto space-y-2 pr-2">{children}</div>
  </div>
);
...
const CommitRow: FC<{ commit: GitCommit }> = ({ commit }) => (
  <div className="font-mono text-xs bg-slate-900/50 p-2 rounded">
    ...
    <div className="text-slate-500 mt-1">
      <span className="text-cyan-400">{commit.author}</span> committed at{" "}
      {commit.timestamp}
    </div>
  </div>
);
...
const WebhookRow: FC<{ webhook: WebhookDelivery }> = ({ webhook }) => (
  <div className="font-mono text-xs flex items-center justify-between bg-slate-900/50 p-2 rounded">
    ...
    <div
      className={`flex items-center gap-2 px-2 py-0.5 rounded-full text-xs ${webhook.success ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}`}
    >
      <div
        className={`w-2 h-2 rounded-full ${webhook.success ? "bg-green-500" : "bg-red-500"}`}
      ></div>
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          Live feed for repository:{" "}
          <span className="font-mono text-cyan-400">agent-zero-vault</span>
        </p>
      </div>
