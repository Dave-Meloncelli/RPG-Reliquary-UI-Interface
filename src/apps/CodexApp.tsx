...
const RuleList: FC<{
  ...
  <div className="space-y-4">
    {Object.entries(rules).map(([category, ruleList]) => (
      <div key={category}>
        <h3 className="text-sm font-bold uppercase text-indigo-300 tracking-wider px-3 pb-2 border-b border-slate-700/50">
          {category}
        </h3>
  ...
  return (
    <div className="p-4 bg-slate-800/50 rounded-lg h-full">
      ...
      <div className="flex items-center gap-4 text-xs text-slate-400 mb-4 pb-2 border-b border-slate-700">
        <span>
          ID: <span className="font-mono text-cyan-400">{rule.id}</span>
        </span>
        <span>
          Ratified:{" "}
          <span className="font-mono text-cyan-400">{rule.ratified}</span>
        </span>
        <span>
          Body:{" "}
          <span className="font-mono text-cyan-400">{rule.ratifyingBody}</span>
        </span>
      </div>
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          The official library of system laws, directives, and protocols.
        </p>
      </div>
