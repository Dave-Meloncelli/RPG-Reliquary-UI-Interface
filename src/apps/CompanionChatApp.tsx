...
const AgentChatList: FC<{
  agents: AgentProfile[];
  selectedAgentId: any | null;
  onSelect: (agent: AgentProfile) => void;
  disabled: boolean;
}> = ({ agents, selectedAgentId, onSelect, disabled }) => (
...
const MessageBubble: FC<{ message: ChatMessage }> = ({ message }) => {
  ...
  return (
    ...
    >
      <div
        className={`rounded-lg p-3 max-w-xl text-sm leading-relaxed shadow-md ${
...
const CompanionChatApp: React.FC = () => {
  ...
  const handleSendMessage = useCallback(
    async (e: React.FormEvent) => {
      ...
      const agentResponsePlaceholder: ChatMessage = {
        sender: "agent",
        text: "",
        timestamp: new Date().toLocaleTimeString(),
      };
            ...
            <div className="p-4 border-t border-slate-700/50 bg-slate-900">
              ...
              >
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-600 text-white font-bold p-2 rounded-md transition"
                >
                  <svg
                    ...
                    className="w-5 h-5"
                  >
                    <path d="M3.105 2.289a.75.75 0 00-.826.95l1.414 4.949a.75.75 0 00.95.826L11.25 9.25v1.5L4.643 12.02a.75.75 0 00-.826.95l1.414 4.949a.75.75 0 00.95.826L16.25 12.25a.75.75 0 000-1.5L3.105 2.289z" />
                  </svg>
                </button>
              </form>
            </div>
