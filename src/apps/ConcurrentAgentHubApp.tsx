...
const TaskPriorityBadge: FC<{ priority: TaskPriority }> = ({ priority }) => {
  ...
  return (
    <span
      className={`px-2 py-1 rounded-full text-xs font-bold ${colors[priority]}`}
    >
      {priority}
    </span>
  );
...
const TaskStatusBadge: FC<{ status: ConcurrentTask["status"] }> = ({
  status,
}) => {
  const colors = {
  ...
  return (
    <span
      className={`px-2 py-1 rounded-full text-xs font-bold ${colors[status]}`}
    >
      {status.toUpperCase()}
    </span>
  );
...
const AgentWorkloadCard: FC<{
  workload: AgentWorkload;
  agent: AgentProfile;
}> = ({ workload, agent }) => {
  const loadPercentage = Math.round(workload.currentLoad * 100);
  const loadColor =
    workload.currentLoad > 0.8
      ? "bg-red-500"
      : workload.currentLoad > 0.5
        ? "bg-yellow-500"
        : "bg-green-500";
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="min-w-0 flex-grow">
          ...
          <p className="text-xs text-slate-400">
            {workload.currentTasks.length}/{workload.maxConcurrentTasks} tasks
          </p>
        </div>
...
const TaskCard: FC<{ task: ConcurrentTask; agent: AgentProfile }> = ({
  task,
  agent,
}) => {
  const duration =
...
const ConcurrentAgentHubApp: FC = () => {
  ...
  const handleSubmitMultipleTasks = async () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const randomPriority = ["Low", "Medium", "High", "Critical"][
  ...
  const getAgentById = (agentId: any) => agents.find((a) => a.id === agentId);
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          Manage multiple agents working on different tasks simultaneously
        </p>
      </div>
