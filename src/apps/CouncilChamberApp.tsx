...
import { runDeliberation } from '../services/councilService';
import type { Agent, CouncilMessage } from '../types';

const AgentAvatar: React.FC<{ agent: Agent }> = ({ agent }) => {
    const baseClasses = "w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-md flex-shrink-0";
    const agentStyles: Record<Agent, { initials: string, color: string }> = {
...
const CouncilChamberApp: React.FC = () => {
    ...
    const handleFormSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        const initialMessages: CouncilMessage[] = [{ agent: 'System', text: `The council will now deliberate on: "${topic}"` }];
        ...
        <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
            <div className="flex-grow p-4 overflow-y-auto space-y-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-3 ${msg.agent === 'System' ? 'justify-center' : ''}`}>
                        {msg.agent !== 'System' && <AgentAvatar agent={msg.agent} />}
                        <div className={`rounded-lg p-3 max-w-xl ${
                            msg.agent === 'System' ? 'bg-slate-700 text-slate-400 italic text-sm' : 'bg-slate-800 shadow-lg'
                        }`}>
                            {msg.agent !== 'System' && <p className="font-bold text-sm mb-1 text-cyan-400">{msg.agent}</p>}
                             <p className="whitespace-pre-wrap text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: msg.text.replace(/\*\*Verdict:(.*?)\*\*/g, '<strong class="text-yellow-400 font-semibold">Verdict:$1</strong>') }} />
                        </div>
                    </div>
                ))}
                ...
                <form onSubmit={handleFormSubmit}>
                    <div className="flex items-center space-x-2">
                        <input
                            ...
                            placeholder={isDeliberating ? "Deliberation in progress..." : "Enter a topic for deliberation..."}
