...
const TABS: Array<keyof CuratedData> = [
  "bookList",
  "priceHistory",
  "series",
  "gossipAndLore",
...
const BookListTable: FC<{
  books: CuratedBook[];
  onSelectionChange: (selectedIsbns: any[]) => void;
}> = ({ books, onSelectionChange }) => {
...
const CuratorApp: React.FC = () => {
  ...
  const [selectedTarget, setSelectedTarget] = useState<CuratorTarget | null>(
    null,
  );
  ...
  useEffect(() => {
    const unsubscribe = curatorService.subscribe((state) => {
      ...
      if (!selectedTarget && allTargets.length > 0) {
        ...
        const updatedTarget = allTargets.find(
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          Manage proactive intelligence gathering and data silos.
        </p>
      </div>
      <div className="flex-grow flex min-h-0">
        ...
        <div className="w-1/3 min-w-[300px] p-4 border-r border-slate-700/50 flex flex-col gap-4">
          <form onSubmit={handleAddTarget} className="flex gap-2">
            ...
            <button
              type="submit"
              className="p-2 bg-indigo-600 hover:bg-indigo-500 rounded-md text-sm font-bold"
            >
              +
            </button>
          </form>
          <div className="flex-grow overflow-y-auto space-y-2 pr-2">
            {targets.map((target) => (
              ...
              >
                {target.isNewDiscovery && (
                  <span className="absolute top-1 right-1 text-xs bg-yellow-500 text-black font-bold px-2 rounded-full">
                    New
                  </span>
                )}
                <p className="font-bold text-white truncate">{target.name}</p>
                <p
                  className={`text-xs ${target.isNewDiscovery ? "text-yellow-300" : "text-slate-400"}`}
                >
                  {target.isNewDiscovery
                    ? "Needs Validation"
                    : `Last Scraped: ${target.lastScraped}`}
                </p>
              </button>
            ))}
          </div>
            ...
            <>
                ...
                <div className="p-3 mb-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-yellow-400">
                      Validate this source to add it to regular monitoring.
                    </p>
                  </div>
                  <button
                    onClick={() =>
                      curatorService.validateTarget(selectedTarget.id)
                    }
              ...
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Next Scrape: {selectedTarget.nextScrape}
                  </p>
                </div>
                <button
                  onClick={() => curatorService.removeTarget(selectedTarget.id)}
                  className="text-xs text-red-400 hover:bg-red-500/20 p-2 rounded-md"
                >
                  Remove
                </button>
              </div>
              <div className="flex border-b border-slate-700">
                {TABS.map((tab) => (
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-semibold transition-colors ${activeTab === tab ? "text-white border-b-2 border-indigo-500" : "text-slate-400 hover:text-white"}`}
                  >
                    {TAB_NAMES[tab]}
                  </button>
                ))}
              ...
              {activeTab === "bookList" && (
                <div className="mt-2">
                  <button
                    ...
                    disabled={
                      selectedBookIsbns.length === 0 ||
                      placeholderStatus !== "idle"
                    }
                  ...
                  >
                    {placeholderStatus === "running"
                      ? "Processing..."
                      : placeholderStatus === "success"
                        ? "Placeholders Created!"
                        : `Create Placeholder Listings (${selectedBookIsbns.length})`}
                  </button>
                </div>
              )}
              <div className="flex-grow bg-black/20 mt-2 rounded-md p-2 overflow-y-auto">
                {!curatedData ? (
                  ...
                  <BookListTable
                    books={curatedData.bookList}
                    onSelectionChange={setSelectedBookIsbns}
                  />
                ) : (
                  <pre className="whitespace-pre-wrap font-mono text-xs p-2">
                    {JSON.stringify(curatedData[activeTab], null, 2)}
                  </pre>
                )}
              </div>
            </>
