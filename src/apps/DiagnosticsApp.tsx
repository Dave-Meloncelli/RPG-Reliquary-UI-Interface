      ...
      <div className="flex-grow overflow-y-auto space-y-1 pr-2">
        {logs.map((log) => (
          <div key={log.id} className="font-mono text-xs">
            ...
            <span
              className={`font-bold mr-2 uppercase ${LOG_STATUS_STYLES[log.status]}`}
            >{`[${log.status}]`}</span>
...
const DiagnosticsApp: FC = () => {
  ...
  const [channels] = useState([
    ...
    { id: "channel-2", name: "Technical" },
    { id: "channel-3", name: "Consciousness" },
  ]);
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          Run on-demand checks to verify system integrity.
        </p>
      </div>

      <div className="flex flex-col md:flex-row h-full">
        <div className="flex-grow">
          <label htmlFor="test-select" className="block text-sm font-bold mb-1">
            Select Test
          </label>
          <select
            id="test-select"
        ...
        <div className="flex items-end">
          <button
            ...
            disabled={isRunning}
            className="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-md transition-colors duration-200"
          >
