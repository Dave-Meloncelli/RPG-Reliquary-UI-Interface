...
import type {
  ErduLogEntry,
  SystemStatus,
  IncidentResponseProgress,
  LogLevel,
  OperationStep,
} from "../types";

const LOG_LEVEL_STYLES: Record<LogLevel, { text: any; bg: any; border: any }> =
  {
    INFO: {
      text: "text-blue-400",
      bg: "bg-blue-900/20",
...
const StatusMeter: FC<{ label: any; value: any; colorClass: any }> = ({
  label,
  value,
  colorClass,
}) => (
  ...
  <div className="h-full bg-black/30 rounded-md p-2 flex flex-col-reverse overflow-y-auto">
    <div className="space-y-2 pr-2">
      {logs.map((log) => (
        <div
          key={log.id}
          className={`p-2 rounded border text-xs font-mono ${LOG_LEVEL_STYLES[log.level].bg} ${LOG_LEVEL_STYLES[log.level].border}`}
        >
          <span className="mr-2 text-slate-500">{log.timestamp}</span>
          <span
            className={`mr-2 font-bold ${LOG_LEVEL_STYLES[log.level].text}`}
          >{`[${log.level}]`}</span>
...
const ErduMonitorApp: FC = () => {
  ...
  const [incidentProgress, setIncidentProgress] =
    useState<IncidentResponseProgress | null>(null);
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      <div className="p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">
          Emergency Response & Defense Unit Live Feed
        </p>
      </div>
          ...
          <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50">
            ...
            <div className="space-y-4">
              ...
              <StatusMeter
                ...
                colorClass={threatColor.replace("text-", "bg-")}
              />
            </div>
          </div>
              ...
              {incidentProgress?.steps.map((step, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <div className="flex-grow">
                    ...
                    {step.result && (
                      <p className="text-xs text-slate-300 mt-1 pl-2 border-l-2 border-slate-700">
                        {step.result}
                      </p>
                    )}
                    {step.error && (
