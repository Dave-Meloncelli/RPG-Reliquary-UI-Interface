...
const MetricCard: FC<{ title: any; value: any; className?: any }> = ({
  title,
  value,
  className = "",
}) => (
...
const LogPanel: FC<{ title: any; children: React.ReactNode }> = ({
  title,
  children,
}) => (
  <div className="bg-black/30 rounded-lg flex flex-col h-full overflow-hidden">
    <h3 className="text-md font-bold text-indigo-300 p-3 bg-slate-900/50">
      {title}
    </h3>
    <div className="flex-grow overflow-y-auto p-3 space-y-2">{children}</div>
  </div>
);
    ...
    <div className="flex flex-col h-full bg-slate-900/80 text-slate-200 font-sans">
      ...
      <div className="flex-grow p-4 pt-0 grid grid-cols-2 gap-4 min-h-0">
        <LogPanel title="LLM Usage Stream">
          {llmLogs.map((log) => (
            <LLMCallLogRow key={log.id} log={log} />
          ))}
        </LogPanel>
        <LogPanel title="Agent Task Log">
          {agentTasks.map((task) => (
            <AgentTaskLogRow key={task.id} log={task} />
          ))}
        </LogPanel>
      </div>
    </div>
