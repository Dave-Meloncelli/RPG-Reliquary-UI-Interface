...
const RPGCommunityHubApp: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'home' | 'publishers' | 'series' | 'books' | 'lore' | 'community'>('home');
  ...
  const initializeSampleData = () => {
    const samplePublishers: Publisher[] = [
      {
        ...
        description: 'The legendary publisher behind Dungeons & Dragons, the world\'s most popular tabletop roleplaying game.',
        founded: '1990',
        ...
        series: [
          {
            ...
            description: 'The world\'s greatest roleplaying game. Create heroic characters, embark on epic adventures, and tell legendary stories.',
            bookCount: 456,
            firstPublished: '1974',
            lastPublished: '2024',
            genre: 'Fantasy',
      ...
      },
      {
        ...
        description: 'Creators of Pathfinder, the award-winning fantasy roleplaying game that puts you in the role of a brave adventurer.',
        founded: '2002',
      ...
      },
      {
        id: 'chaosium',
        name: 'Chaosium Inc',
        description: 'Publishers of Call of Cthulhu, the horror roleplaying game based on the works of H.P. Lovecraft.',
        founded: '1975',
        ...
        series: [
          {
            ...
            description: 'A horror roleplaying game based on the works of H.P. Lovecraft, where investigators confront cosmic horrors.',
            bookCount: 189,
            firstPublished: '1981',
            lastPublished: '2024',
            genre: 'Horror',
    ...
    const sampleBooks: RPGBook[] = [
      {
        ...
        isbn: '978-0786965601',
        coverImage: '📚',
        description: 'The essential reference for every Dungeons & Dragons roleplayer. Contains rules for character creation, combat, magic, and more.',
        genre: 'Fantasy',
        ...
        inStock: true,
        affiliateLinks: [
          { platform: 'Amazon', url: '#', price: 49.95, commission: 2.50, inStock: true },
          { platform: 'Barnes & Noble', url: '#', price: 49.95, commission: 2.00, inStock: true },
          { platform: 'Local Game Store', url: '#', price: 49.95, commission: 5.00, inStock: true }
        ],
        communityRating: 4.8,
        reviewCount: 1247
      },
      {
        ...
        isbn: '978-1640781689',
        coverImage: '📚',
        description: 'The complete rules for the Pathfinder roleplaying game, including character creation, combat, magic, and equipment.',
        genre: 'Fantasy',
        setting: 'Golarion',
        complexity: 'advanced',
        pageCount: 640,
        ...
        inStock: false,
        affiliateLinks: [
          { platform: 'Amazon', url: '#', price: 59.99, commission: 3.00, inStock: true },
          { platform: 'Paizo Store', url: '#', price: 59.99, commission: 6.00, inStock: true }
        ],
        communityRating: 4.7,
        reviewCount: 892
      }
    ...
    const sampleLore: LoreArticle[] = [
      {
        id: 'dnd-history',
        title: 'The Evolution of Dungeons & Dragons: From Chainmail to 5th Edition',
        category: 'history',
        content: 'Dungeons & Dragons began as a medieval wargame called Chainmail in 1971...',
      ...
      },
      {
        ...
        category: 'culture',
        content: 'Roleplaying games have always been a reflection of both the best and worst aspects of human nature...',
        author: 'The Reliquarian',
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-blue-200">
            The ultimate destination for RPG enthusiasts. Explore publishers, discover lore, and find your next adventure.
          </p>
        </div>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-black/30 rounded-lg p-6">
                <h2 className="text-2xl font-bold mb-4">🌟 Featured Publisher</h2>
                <div className="text-center">
                  <div className="text-6xl mb-4">🎲</div>
                  <h3 className="text-xl font-semibold mb-2">Wizards of the Coast</h3>
                  <p className="text-gray-300 mb-4">The legendary publisher behind Dungeons & Dragons</p>
                  <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-black/20 p-6 rounded-lg text-center">
                <div className="text-3xl font-bold text-blue-400">{publishers.length}</div>
                <div className="text-sm text-gray-400">Publishers</div>
              </div>
              <div className="bg-black/20 p-6 rounded-lg text-center">
                <div className="text-3xl font-bold text-green-400">{books.length}</div>
                <div className="text-sm text-gray-400">Books</div>
              </div>
              <div className="bg-black/20 p-6 rounded-lg text-center">
                <div className="text-3xl font-bold text-purple-400">{loreArticles.length}</div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {publishers.map(publisher => (
                <div key={publisher.id} className="bg-black/30 rounded-lg p-6 hover:bg-black/40 transition-colors cursor-pointer"
                     onClick={() => setSelectedPublisher(publisher)}>
                  <div className="text-center mb-4">
                    <div className="text-4xl mb-2">{publisher.logo}</div>
                    <h3 className="text-lg font-semibold">{publisher.name}</h3>
                  </div>
                  <p className="text-sm text-gray-300 mb-4">{publisher.description}</p>
                  <div className="flex justify-between text-sm">
                    <span className="text-blue-400">{publisher.bookCount} books</span>
                    <span className="text-yellow-400">★ {publisher.averageRating}</span>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={book.id} className="bg-black/30 rounded-lg p-6">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm text-gray-400">{book.series} ({book.edition})</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <span className={`px-2 py-1 rounded text-xs ${getComplexityColor(book.complexity)}`}>
                      {book.complexity.toUpperCase()}
                    </span>
                    <span className="text-sm text-gray-400">{book.genre}</span>
                    <span className="text-sm text-gray-400">★ {book.communityRating}</span>
                  </div>
                    ...
                    {book.affiliateLinks.map((link, index) => (
                      <div key={index} className="flex justify-between items-center bg-black/20 p-2 rounded">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {link.inStock ? (
                            <span className="text-xs bg-green-600 px-2 py-1 rounded">In Stock</span>
                          ) : (
                            <span className="text-xs bg-red-600 px-2 py-1 rounded">Out of Stock</span>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              {loreArticles.map(article => (
                <div key={article.id} className="bg-black/30 rounded-lg p-6">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-1 bg-blue-600 rounded text-xs">{article.category}</span>
                    {article.featured && <span className="px-2 py-1 bg-yellow-600 rounded text-xs">Featured</span>}
                  </div>
                  <h3 className="text-lg font-semibold mb-2">{article.title}</h3>
                  <p className="text-sm text-gray-400 mb-4">By {article.author} • {article.publishDate}</p>
          ...
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-slate-800 rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-gray-300 mb-4">{selectedPublisher.description}</p>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{selectedPublisher.headquarters}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {selectedPublisher.series.map(series => (
                  <div key={series.id} className="bg-black/20 p-4 rounded">
                    ...
                    <p className="text-sm text-gray-300 mb-2">{series.description}</p>
                    <div className="flex gap-2 text-xs">
                      <span className="px-2 py-1 bg-blue-600 rounded">{series.genre}</span>
                      <span className="px-2 py-1 bg-green-600 rounded">{series.bookCount} books</span>
                      <span className={`px-2 py-1 rounded ${getComplexityColor(series.complexity)}`}>
                        {series.complexity}
                      </span>
                    </div>
                  </div>
