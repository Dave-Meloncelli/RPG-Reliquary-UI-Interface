...
const PRIORITY_STYLES: Record<TaskPriority, { text: string; bg: string }> = {
    Low: { text: 'text-sky-300', bg: 'bg-sky-500/20' },
    Medium: { text: 'text-yellow-300', bg: 'bg-yellow-500/20' },
    High: { text: 'text-orange-400', bg: 'bg-orange-500/20' },
    Critical: { text: 'text-red-400', bg: 'bg-red-500/20' },
};

const STATUS_STYLES: Record<TaskStatus, { text: string; bg: string }> = {
    Pending: { text: 'text-yellow-300', bg: 'bg-yellow-500/20' },
    Resolved: { text: 'text-green-300', bg: 'bg-green-500/20' },
};

const TaskCard: FC<{ task: TaskItem, onResolve: (id: string) => void, onOpen: (appId: string) => void }> = ({ task, onResolve, onOpen }) => {
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 space-y-3">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-slate-700/50">
                <div className="flex items-center gap-4 text-xs text-slate-400">
                    <span>Source: <span className="font-semibold text-indigo-300">{task.source}</span></span>
                ...
                <div className="flex gap-2">
                    <button onClick={() => onOpen(task.appId)} className="px-3 py-1 bg-sky-600/50 text-sky-300 rounded-md text-xs font-semibold hover:bg-sky-600 hover:text-white transition-colors">
                        Go to App
                    </button>
                    {task.status === 'Pending' && (
                        <button onClick={() => onResolve(task.id)} className="px-3 py-1 bg-green-600/50 text-green-300 rounded-md text-xs font-semibold hover:bg-green-600 hover:text-white transition-colors">
                            Resolve
                        </button>
                    )}
                </div>
...
const TaskReviewHubApp: React.FC = () => {
