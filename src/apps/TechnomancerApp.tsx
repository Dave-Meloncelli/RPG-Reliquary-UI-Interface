...
import { technomancerService } from '../services/technomancerService';
import type { MonitoredTechnology, TechUpdateLog, FaultFixRecord } from '../types';

const TABS = ['Dashboard', 'Patches & Updates', 'Faults & Fixes'];

const STATUS_STYLES: Record<MonitoredTechnology['status'], { text: string; bg: string; }> = {
    'Up-to-date': { text: 'text-green-300', bg: 'bg-green-500/20' },
    'Update Available': { text: 'text-yellow-300', bg: 'bg-yellow-500/20' },
    'Vulnerable': { text: 'text-red-300', bg: 'bg-red-500/20' },
    'Unknown': { text: 'text-slate-400', bg: 'bg-slate-500/20' },
};

const IMPORTANCE_STYLES: Record<TechUpdateLog['importance'], string> = {
...
const TechnomancerApp: React.FC = () => {
                             ...
                             <div key={log.id} className={`p-3 rounded-lg bg-slate-800/50 border-l-4 ${IMPORTANCE_STYLES[log.importance]}`}>
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400">{log.timestamp} &bull; Importance: {log.importance}</p>
                                    </div>
                                     <a href={log.changelogUrl} target="_blank" rel="noopener noreferrer" className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-1 px-3 rounded-md">Changelog</a>
                                </div>
                    ...
                    <div className="space-y-4">
                         {state.faultFixRecords.map(record => (
                             <div key={record.id} className="p-3 bg-slate-800/50 rounded-lg">
                                ...
                                <p className="font-semibold text-red-400 mb-1">Fault: <span className="text-slate-300">{record.fault}</span></p>
                                <p className="font-semibold text-green-400">Resolution: <span className="text-slate-300">{record.resolution}</span></p>
