...
const AgentStatusSummaryWidget: FC = () => {
  ...
  useEffect(() => {
    const updateState = (
      state: ReturnType<typeof controlPanelService.getState>,
    ) => {
      const masterStatus = state.agentMasterStatus;
      const total = Object.keys(masterStatus).length;
      const online = Object.values(masterStatus).filter(
        (s) => s === "Online",
      ).length;
  ...
  return (
    <div className="h-full flex items-center justify-around text-center">
      ...
      <div>
        <p className="text-4xl font-bold font-mono text-red-500">
          {status.dormant}
        </p>
