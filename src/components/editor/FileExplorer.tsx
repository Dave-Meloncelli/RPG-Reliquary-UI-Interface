...
const FileOrFolder: FC<FileExplorerProps & { depth: number }> = ({ node, onFileSelect, basePath, depth }) => {
...
const FileExplorer: FC<FileExplorerProps> = ({ node, onFileSelect, basePath }) => {
    // We only want to render the children of the root 'root' node.
    return (
        <div className="p-2 text-sm">
            {node.children?.map(child => (
                <FileOrFolder key={child.name} node={child} onFileSelect={onFileSelect} basePath="" depth={0} />
            ))}
        </div>
    );
};
