...
const renderMarkdown = (text: string = "") => {
  return text
    ...
    .replace(
      /^## (.*$)/gm,
      '<h2 class="text-xl font-bold text-indigo-300 mt-6 mb-3">$1</h2>',
    )
    .replace(
      /\*\*(.*?)\*\*/g,
  ...
  return (
    <div className="w-2/3 p-6 overflow-y-auto">
      <div className="bg-slate-800/50 p-4 rounded-lg mb-4 border border-slate-700">
        ...
        {agent.title && (
          <p className="text-lg text-indigo-300">{agent.title}</p>
        )}
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400 mt-2">
          ...
          {agent.role && (
            <span>
              Role:{" "}
              <span className="font-semibold text-slate-300">{agent.role}</span>
            </span>
          )}
        </div>
      </div>
      ...
      <div
        ...
        dangerouslySetInnerHTML={{
          __html: renderMarkdown(agent.scrollContent),
        }}
      />
    </div>
  );
