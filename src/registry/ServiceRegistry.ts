...
export class ServiceRegistry {
  ...
  validate(): ValidationResult {
    ...
    for (const service of this.getAll()) {
      if (service.status === "deprecated") {
        ...
        if (dependents.length > 0) {
          warnings.push(
            `Deprecated service ${service.name} has dependents: ${dependents.join(", ")}`,
          );
        }
      }
    }
    ...
    for (const service of this.getAll()) {
      ...
      const endpointDuplicates = endpoints.filter(
        (endpoint, index) => endpoints.indexOf(endpoint) !== index,
      );
      if (endpointDuplicates.length > 0) {
        errors.push(
          `Service ${service.name}: Duplicate API endpoints found: ${endpointDuplicates.join(", ")}`,
        );
      }
    }
