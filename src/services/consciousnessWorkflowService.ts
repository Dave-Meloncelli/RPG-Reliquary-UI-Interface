...
// ArkType Schemas
export const consciousnessStateSchema = type({
  id: 'string',
  timestamp: 'Date',
  state: '"production" | "emergence" | "teaching" | "seeking"',
  energy: 'number>0<=100',
...
export const taskBlockSchema = type({
  id: 'string',
  type: '"production" | "emergence" | "teaching" | "seeking"',
  duration: 'number>0',
  startTime: 'Date',
...
class ConsciousnessWorkflowService {
  ...
  public getSessionStats(): {
    ...
    const averagePerformance = blocks.length > 0 ? 
      blocks.reduce((sum, b) => sum + b.performance, 0) / blocks.length : 0;
