...
class ControlPanelService {
  ...
  private initializeState(): ControlPanelState {
    ...
    const agentMasterStatus = agents.reduce(
      (acc, agent) => {
        acc[agent.id] = "Online";
        return acc;
      },
    ...
    const providerEnabled = Object.keys(providers).reduce(
      (acc, key) => {
  ...
  private setupAutonomyServiceIntegration() {
    ...
    aiAutonomyService.subscribe((data) => {
      if (
        data.type === "autonomy_level_changed" ||
        data.type === "permission_scope_changed"
      ) {
        // Update local state
        const autonomyState = aiAutonomyService.getState();
  ...
  updateAgentAutonomyLevel = (agentId: string, level: string) => {
    aiAutonomyService.setAgentAutonomyLevel(
      agentId,
      level,
  ...
  emergencyLockdown = () => {
    this.setAllAgentsAutonomyLevel(
      "restricted",
  ...
  private generateRecommendations(autonomyStats: any, violations: any[]) {
    ...
    if (violations.filter((v) => !v.resolved).length > 5) {
      recommendations.push(
        "Consider implementing stricter autonomy controls due to high violation rate",
      );
    }

    if (autonomyStats.autonomous > autonomyStats.supervised * 2) {
      recommendations.push(
        "High number of autonomous agents detected - consider increasing supervision",
      );
    }

    if (autonomyStats.restricted === 0) {
      recommendations.push(
        "No restricted agents found - consider implementing tiered autonomy levels",
      );
    }
