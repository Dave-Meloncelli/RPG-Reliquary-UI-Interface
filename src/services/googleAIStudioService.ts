...
export class GoogleAIStudioService {
   ...
   */
  async generateText(request: Omit<GenerationRequest, 'id'>): Promise<GenerationResponse> {
    ...
    try {
      ...
      const generationResponse: GenerationResponse = {
        ...
        safetyRatings: data.candidates?.[0]?.safetyRatings?.map((rating: any) => ({
   ...
   */
  async generateImage(request: Omit<ImageGenerationRequest, 'id'>): Promise<ImageGenerationResponse> {
    if (!this.isConnected || !this.apiKey) {
      throw new Error('Not connected to Google AI Studio');
    }
   ...
   */
  async generateCode(request: Omit<CodeGenerationRequest, 'id'>): Promise<CodeGenerationResponse> {
    if (!this.isConnected || !this.apiKey) {
      throw new Error('Not connected to Google AI Studio');
    }
