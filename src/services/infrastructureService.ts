...
const initialStatus: InfrastructureStatus = {
  docker: [
    ...
    {
      id: "docker-python-nlp",
      name: "python-nlp",
      image: "local/python-nlp",
      status: "running",
    },
...
export const simulateInfrastructureUpdates = (
  current: InfrastructureStatus,
): InfrastructureStatus => {
  ...
  updated.docker = updated.docker.map((service) => {
    if (Math.random() < 0.02 && service.status !== "error") {
      // 2% chance to flicker
      setTimeout(() => {
