...
export class RPGDatabaseService {
  ...
  private async extractMetadataFromPDF(
    filePath: string,
  ...
  private determineDocumentType(
    ...
    if (
      textLower.includes("character creation") ||
  ...
  private async extractLoreFromText(
    text: string,
    documentId: string,
  ): Promise<LoreEntry[]> {
  ...
  async searchImages(
    ...
    for (const [documentId, document] of this.documents) {
      for (const image of document.extractedImages) {
        ...
        if (
          filters?.usageRights &&
          !filters.usageRights.includes(image.usageRights)
        ) {
          continue;
        }
  ...
  async searchLore(
    ...
    for (const [documentId, document] of this.documents) {
      for (const lore of document.lore) {
        ...
        if (
          lore.title.toLowerCase().includes(queryLower) ||
  ...
  async generateCharacterSheet(
    system: string,
    theme: string,
    accessibility: "standard" | "neurodiverse",
  ): Promise<string> {
    // Generate character sheet based on system and accessibility needs
    const template = await this.getCharacterSheetTemplate(
      system,
      accessibility,
    );
  ...
  private async getCharacterSheetTemplate(
    system: string,
    ...
    const templates: Record<string, Record<string, string>> = {
      ...
      pathfinder: {
        ...
        neurodiverse:
          "Neurodiverse-friendly Pathfinder character sheet template",
      },
    };

    return (
      templates[system]?.[accessibility] || "Generic character sheet template"
    );
  }

  private async applyThemeToTemplate(
    template: string,
    theme: string,
  ): Promise<string> {
